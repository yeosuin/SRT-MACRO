(() => {
    ...
    const init = () => {
        document.getElementById('bot-token').value = localStorage.getItem('SRT_MACRO::bot-token');
        document.getElementById('chat-id').value = localStorage.getItem('SRT_MACRO::chat-id');
    }
    ...
    const save = () => {
        ...
        fetch(url).then(response => {
            if (response.status === 200) {
                localStorage.setItem('SRT_MACRO::bot-token', botToken);
                localStorage.setItem('SRT_MACRO::chat-id', chatId);
    ...
    const reset = () => {
        ...
        localStorage.removeItem('SRT_MACRO::bot-token');
        localStorage.removeItem('SRT_MACRO::chat-id');
